...
function git(cwd: string, ...args: string[]): string {
  ...
  if (result.status !== 0)
    throw new Error(
      (result.stderr || result.error?.message || "git failed").trim(),
    );
...
function windowsShortPath(path: string): string | undefined {
  ...
  return shortPath && shortPath.toLowerCase() !== path.toLowerCase()
    ? shortPath
    : undefined;
}
...
function harness() {
  ...
  spawnSync("git", ["init", "-b", "main", projectRoot], {
    encoding: "utf8",
    windowsHide: true,
  });
...
function seedTask(
  ...
  const project =
    h.plane.listProjects()[0] ??
    h.plane.createProject(
      { name: "P", rootPath: h.projectRoot, maxSlots: 4 },
      10,
    );
  const slot =
    h.plane.listAgentSlots(project.id).find((item) => item.role === role) ??
    h.plane.createAgentSlot(project.id, role, 11);
  ...
  h.plane.work.replace(
    {
      ...
      transportGeneration: `seed-${taskId}`,
      transportSequence: snapshot.revision + 1,
      transportMessageId: `seed-${taskId}-${snapshot.revision + 1}`,
...
describe("WorkspaceAuthority", () => {
  it("provisions an isolated task worktree with durable lease and capability identity", () => {
    ...
    const workspace = h.plane.provisionTaskWorkspace(
      seeded.project.id,
      seeded.slot.id,
      seeded.task.id,
      20,
    ...
    expect(h.plane.getLease(workspace.leaseId)).toMatchObject({
      holderId: seeded.slot.id,
      taskId: seeded.task.id,
      mode: "exclusive",
      status: "active",
    });
    expect(
      h.plane.provisionTaskWorkspace(
        seeded.project.id,
        seeded.slot.id,
        seeded.task.id,
        21,
  ...
  it("accepts a Windows short-name alias for the repository root", () => {
    if (process.platform !== "win32") return;
    const h = harness();
    const shortRoot = windowsShortPath(h.projectRoot);
    if (!shortRoot) return;
    const project = h.plane.createProject(
      { name: "ShortPath", rootPath: shortRoot, maxSlots: 4 },
      10,
    );
    const slot = h.plane.createAgentSlot(project.id, "WORKER", 11);
    const snapshot = h.plane.work.snapshot();
    const task = {
      id: "task-short-path",
      kind: "work",
      completionPolicy: "verified-claim",
      title: "task-short-path",
      project: project.name,
      instruction: "do task-short-path",
    ...
    h.plane.work.replace(
      {
        expectedRevision: snapshot.revision,
        transportGeneration: "seed-short-path",
        transportSequence: snapshot.revision + 1,
        transportMessageId: `seed-short-path-${snapshot.revision + 1}`,
        tasks: [...snapshot.tasks, task],
        attempts: snapshot.attempts,
        messages: snapshot.messages,
      },
  ...
  it("requires the claim commit to be the assigned workspace branch HEAD", () => {
    ...
    expect(
      verification.checks.find((item) => item.name === "workspace.commit")
        ?.passed,
    ).toBe(true);
    expect(h.plane.work.getTask(seeded.task.id)).toMatchObject({
      machineCompletion: {
        claimId: claim.id,
    ...
    expect(h.plane.verifyClaimAndCompleteTask(claim.id, 40).id).toBe(
      verification.id,
    );
  ...
  it("fences verified authority even when physical workspace cleanup must be deferred", () => {
    ...
    const release = vi
      .spyOn(h.plane.workspaces, "release")
      .mockImplementation(() => {
        throw new Error("simulated Windows worktree lock");
      });
    const verification = h.plane.verifyClaimAndCompleteTask(claim.id, 31);
    expect(verification.status).toBe("passed");
    expect(
      h.plane.work.getTask(seeded.task.id)?.machineCompletion,
    ).toMatchObject({ claimId: claim.id, commitSha: head });
    expect(h.plane.getLease(workspace.leaseId).status).toBe("released");
    const capability = h.plane.database.db
      .prepare("SELECT revoked_at FROM capabilities WHERE id=?")
      .get(workspace.capabilityId) as { revoked_at: number | null };
    expect(capability.revoked_at).toBe(31);
    expect(existsSync(workspace.capabilityTokenPath)).toBe(false);
    expect(h.plane.workspaces.get(workspace.id).status).toBe("active");
    expect(
      h.plane
        .listEvents(seeded.project.id, 0, 200)
        .some((event) => event.type === "TASK_WORKSPACE_RELEASE_DEFERRED"),
    ).toBe(true);
    release.mockRestore();
    expect(h.plane.verifyClaimAndCompleteTask(claim.id, 40).id).toBe(
      verification.id,
  ...
  it("converges a verified crash orphan without deleting preserved files", () => {
    ...
    expect(verification.status).toBe("passed");
    h.plane.work.completeVerifiedClaim(
      {
        taskId: seeded.task.id,
        claimId: claim.id,
        verificationId: verification.id,
    ...
    expect(
      h.plane
        .listEvents(seeded.project.id, 0, 200)
        .some((event) => event.type === "TASK_WORKSPACE_ORPHAN_PRESERVED"),
    ).toBe(true);
  });
  ...
  it("refuses to remove a dirty worktree", () => {
    ...
    expect(() => h.plane.releaseTaskWorkspace(workspace.id, 30)).toThrow(
      /dirty.*refusing/i,
    );
