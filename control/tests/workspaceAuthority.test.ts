...
function windowsShortPath(path: string): string | undefined {
  ...
  const result = spawnSync('cmd.exe', ['/d', '/c', `for %I in (${path}) do @echo %~sI`], { encoding: 'utf8', windowsHide: true });
  if (result.status !== 0) return undefined;
  const shortPath = String(result.stdout ?? '').trim();
  return shortPath && shortPath.toLowerCase() !== path.toLowerCase() ? shortPath : undefined;
}
...
describe('WorkspaceAuthority', () => {
  ...
  it('accepts a Windows short-name alias for the repository root', () => {
    ...
    if (!shortRoot) return;
    const project = h.plane.createProject({ name: 'ShortPath', rootPath: shortRoot, maxSlots: 4 }, 10);
    const slot = h.plane.createAgentSlot(project.id, 'WORKER', 11);
    const snapshot = h.plane.work.snapshot();
    const task = {
      id: 'task-short-path', kind: 'work', completionPolicy: 'verified-claim', title: 'task-short-path', project: project.name,
      instruction: 'do task-short-path', targetRole: slot.role, dependsOn: [], attemptIds: [], createdAt: 12, updatedAt: 12,
    };
    h.plane.work.replace({
      expectedRevision: snapshot.revision, transportGeneration: 'seed-short-path', transportSequence: snapshot.revision + 1,
