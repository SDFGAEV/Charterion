...
function sameFilesystemPath(left: string, right: string): boolean {
  ...
  try {
    ...
    // Fall back to case-insensitive path comparison when identity metadata is unavailable.
  }
...
function workspaceFrom(row: Row): TaskWorkspace {
  return {
    ...
    capabilityTokenPath: String(row.capability_token_path),
    status: String(row.status) as TaskWorkspace["status"],
...
export type VerifiedWorkspaceCleanup =
  "removed" | "already-released" | "orphan-preserved";
...
export class WorkspaceAuthority {
  ...
  private validateManagedPath(path: string): string {
    ...
    if (!rel || rel === "." || rel.startsWith("..") || isAbsolute(rel))
      throw new Error("Workspace path escapes GAM managed root");
    return candidate;
  }
  ...
  private repoRoot(projectRoot: string): string {
    ...
    const top = realpathSync(
      this.git(requested, ["rev-parse", "--show-toplevel"]),
    );
    if (!sameFilesystemPath(requested, top)) {
      throw new Error(
        `Project root ${requested} must be the Git repository root ${top}`,
      );
    }
    return top;
  }
  ...
  find(projectId: string, taskId: string): TaskWorkspace | undefined {
    const row = this.database.db
      .prepare("SELECT * FROM task_workspaces WHERE project_id=? AND task_id=?")
  ...
  list(projectId?: string): TaskWorkspace[] {
    const rows = (
      projectId
        ? this.database.db
            .prepare(
              "SELECT * FROM task_workspaces WHERE project_id=? ORDER BY created_at,id",
            )
            .all(projectId)
        : this.database.db
            .prepare("SELECT * FROM task_workspaces ORDER BY created_at,id")
            .all()
    ) as Row[];
  ...
  materialize(input: {
    ...
    if (existing) {
      if (existing.slotId !== input.slotId)
        throw new Error(
          `Task ${input.taskId} workspace belongs to another AgentSlot`,
        );
      if (existing.status !== "active")
        throw new Error(`Task ${input.taskId} workspace is ${existing.status}`);
      return {
    ...
    const path = this.validateManagedPath(
      join(
        this.managedRoot,
        input.projectId,
        `${slug(input.role)}-${slug(input.taskId).slice(0, 32)}`,
      ),
    );
    ...
    if (existsSync(path)) {
      ...
      if (currentBranch !== branch || headSha !== baseSha || dirty) {
        throw new Error(
          `Existing path ${path} is not a clean recoverable GAM worktree`,
        );
      }
    ...
    if (branchProbe.status === 0)
      throw new Error(
        `Workspace branch ${branch} already exists without a durable task workspace record`,
      );
    if (branchProbe.status !== 1)
      throw new Error(
        branchProbe.error?.message ||
          "Unable to inspect workspace branch identity",
      );
    const result = spawnSync(
      this.gitPath,
      ["-C", repoPath, "worktree", "add", "-b", branch, path, baseSha],
      {
        encoding: "utf8",
        windowsHide: true,
        timeout: 30_000,
  ...
  record(
    ...
    try {
      ...
      try {
        rmSync(capabilityTokenPath, { force: true });
      } catch {
        /* keep primary error */
      }
      throw error;
  ...
  observe(id: string): WorkspaceObservation {
    ...
    if (workspace.status !== "active")
      throw new Error(`Task workspace ${id} is ${workspace.status}`);
    ...
    if (branch !== workspace.branch)
      throw new Error(
        `Task workspace ${id} branch drifted from ${workspace.branch} to ${branch}`,
      );
  ...
  private registeredWorktree(repoPath: string, path: string): boolean {
    ...
    return this.git(repoPath, ["worktree", "list", "--porcelain"])
      ...
      .some((line) => {
        ...
        return sameFilesystemPath(actual, wanted);
      });
  }
  ...
  ): { workspace: TaskWorkspace; cleanup: VerifiedWorkspaceCleanup } {
    const workspace = this.get(id);
    if (workspace.status === "released")
      return { workspace, cleanup: "already-released" };
    try {
      const released = this.release(id, now, options);
      return { workspace: released, cleanup: "removed" };
    } catch (error) {
      if (this.registeredWorktree(workspace.repoPath, workspace.path))
        throw error;
      this.database.db
        .prepare(
          "UPDATE task_workspaces SET status='released',updated_at=? WHERE id=?",
        )
        .run(now, id);
  ...
  release(
    ...
    if (observation.dirty)
      throw new Error(
        `Task workspace ${id} is dirty; refusing to remove uncommitted work`,
      );
    let error: Error | undefined;
    const attempts = options.attempts ?? (process.platform === "win32" ? 8 : 1);
    const timeoutMs = options.timeoutMs ?? 15_000;
    if (!Number.isInteger(attempts) || attempts < 1)
      throw new Error("Workspace release attempts must be a positive integer");
    if (!Number.isInteger(timeoutMs) || timeoutMs < 1)
      throw new Error("Workspace release timeoutMs must be a positive integer");
